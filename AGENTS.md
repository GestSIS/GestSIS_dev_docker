...
Détails utiles côté API : les tests tournent en `APP_ENV=testing` (voir `phpunit.xml`), sur une base MySQL dédiée, et la plupart des feature tests passent l'en-tête `Sis-Id` (multi-tenant, voir ci-dessous). Lance le **minimum** de tests ciblés pendant le dev, puis la suite complète avant de finaliser.
...
- **L'API est multi-tenant** : elle sélectionne la base selon l'en-tête `Sis-Id` de la requête (middleware `DbSelector`). En dev, les bases tenant sont listées dans `DB_LISTE` (`hs,test`) du `.env.docker`. Les feature tests reproduisent cela via l'en-tête `Sis-Id`.
